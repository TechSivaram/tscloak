(() => {
  ...
  async function loadMe() {
    ...
    const name =
      user.name || user.preferred_username || user.username || 'Administrator';
    const roles = Array.isArray(user.roles) ? user.roles : [];
    const roleSummary =
      roles.length > 1
        ? `${roles[0]} +${roles.length - 1} roles`
        : roles[0] || 'IDP_CLIENT_ADMIN';
    document
      .querySelectorAll('[data-user-name]')
      .forEach((node) => (node.textContent = name));
    document
      .querySelectorAll('[data-user-role]')
      .forEach((node) => (node.textContent = roleSummary));
    ...
    document.querySelectorAll('.user-chip').forEach((chip) => {
      ...
      chip
        .querySelector('[data-header-logout]')
  ...
  document.querySelectorAll('.sidebar nav').forEach((nav) => {
    if (!nav.querySelector('a[href="./settings.html"]')) {
      ...
      link.textContent = 'Client settings';
