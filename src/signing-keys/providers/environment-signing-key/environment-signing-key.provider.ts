...
export class EnvironmentSigningKeyProvider implements SigningKeyProvider {
  ...
  getPrivateJwks(): PrivateJwks {
    const value = this.config.get<string>('OIDC_JWKS');

    if (!value) {
      throw new Error('OIDC_JWKS environment variable is not configured');
    }

    try {
      return JSON.parse(value) as PrivateJwks;
    } catch {
      throw new Error('OIDC_JWKS contains invalid JSON');
    }
  }
}
