...
export class OidcAdapter {
  ...
  async find(id: string): Promise<Record<string, unknown> | undefined> {
    const oidc = await this.repository.find(this.model, id);

    return this.getValidPayload(oidc);
  }

  async destroy(id: string): Promise<void> {
    await this.repository.delete(this.model, id);
  }
  ...
  async findByUid(uid: string): Promise<Record<string, unknown> | undefined> {
    const oidc = await this.repository.findByUid(this.model, uid);
  ...
  async findByUserCode(
    ...
    const oidc = await this.repository.findByUserCode(this.model, userCode);

    return this.getValidPayload(oidc);
  }

  async revokeByGrantId(grantId: string): Promise<void> {
    const records = await this.repository.findByGrantId(grantId);

    for (const record of records) {
      await this.repository.delete(record.model, record.id);
    }
  }
  ...
  private async getValidPayload(
    ...
    if (oidc.expiresAt && oidc.expiresAt.getTime() <= Date.now()) {
      await this.repository.delete(oidc.model, oidc.id);
