...
export class ClientsService {
  ...
  async findClientForCallback(callbackPath: string): Promise<Client | null> {
  ...
  async findByClientId(clientId: string): Promise<Client | null> {
    ...
    if (client) {
      ...
      client.postLogoutRedirectUris =
        this.resolvePostLogoutRedirectUris(client);
    }

    return client;
  }

  private resolveRedirectUris(client: Client): string[] {
    ...
    const idpClientAdminCallback =
      `${issuer.replace(/\/+$/, '')}` + `/idp-client-admin/callback.html`;

    return [...new Set([...registered, idpClientAdminCallback])];
  }

  private resolvePostLogoutRedirectUris(client: Client): string[] {
    ...
    return [...new Set([...registered, idpClientAdminUri])];
  }
  ...
  async updateClient(
    ...
    if (input.interactionLoginUrl !== undefined)
      client.interactionLoginUrl = input.interactionLoginUrl || null;
    if (input.interactionConsentUrl !== undefined)
  ...
  async createClient(input: CreateClientInput): Promise<CreatedClient> {
    const clientId = randomBytes(24).toString('hex');

    const existing = await this.clients.findByClientId(clientId);

    if (existing) {
      throw new ConflictException('Client ID collision');
    }
